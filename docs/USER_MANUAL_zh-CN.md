...
- `desktop/Vibrary 0.1.8.exe`：Windows 桌面程序，双击运行。
...
```powershell
Get-FileHash -Algorithm SHA256 ".\desktop\Vibrary 0.1.8.exe"
...
9. Windows 后端会自动处理索引队列；桌面端 `传输` 页面会显示上传和索引状态。
...
双击 `Vibrary 0.1.8.exe` 后，桌面端会自动启动两个隐藏 sidecar：
...
- `首页`：查看 Qdrant、Backend、局域网 API、数据目录、资料总数、图片数量、索引数量、可信设备和队列概况。
- `资料中心`：查看所有进入资料组的文件。图片会显示缩略图；每项会显示文件类型、大小、来源设备、索引状态和打开入口。
- `导入`：从 Windows 选择文件或文件夹，导入到资料库正本并进入索引队列。
- `搜索`：搜索已经完成索引的资料。图片结果会显示缩略图。
- `传输`：查看 Android 上传队列和 Windows 后端索引队列，并可手动触发 `处理索引`。
- `设备`：显示手机加入用的 6 位验证码、局域网服务地址和可信设备列表。
- `设置`：调整局域网连接、自动发现广播、自动索引、缓存和模型相关状态。
...
`传输` 页面中的 `上传队列` 显示 Android 上传到 Windows 的任务。常见状态：
...
`传输` 页面中的 `索引队列` 显示等待、正在处理或已完成的索引任务。
...
- `设置 / Embedding 阶段`：显示当前 embedding provider。当前可选项为 `jina-embeddings-v5-omni-small`，后续版本可以在同一位置增加更多模型。
- `设置 / 检索阶段`：在 `HNSW 向量索引` 和 `遍历 / 精确扫描` 之间切换。遍历模式仍由 Qdrant 执行 exact vector search，不是 SQLite 文件名搜索。
- `设置 / HNSW 参数`：调整 `m`、`ef_construct`、`full_scan_threshold` 和查询时的 `hnsw_ef`。
- `设置 / 索引控制`：查看资料总数、已索引数量、等待队列、失败队列；可以手动 `处理索引` 或 `重构全部索引`。
- `设置 / Qdrant Collection`：查看当前文本、图片语义、图片标签 collection 名称，用于确认搜索是否在预期的 Qdrant collection 上执行。
- `设置 / 连接设置`：提供局域网连接开关、自动发现广播开关和自动索引开关。关闭局域网连接后，手机将无法发现或访问电脑端。

设置会保存到本机配置目录。切换局域网、自动索引、embedding provider、检索方式或 HNSW 参数后，桌面端会自动重启后台服务以应用新设置。切换 embedding 或 HNSW collection 参数后，建议在 `设置 / 索引控制` 中点击 `重构全部索引`。

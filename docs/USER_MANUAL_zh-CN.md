...
- `desktop/Vibrary 0.1.9.exe`：Windows 桌面程序，双击运行。
...
```powershell
Get-FileHash -Algorithm SHA256 ".\desktop\Vibrary 0.1.9.exe"
...
2. 双击 `Vibrary 0.1.9.exe`。
3. 等待顶部状态区显示 Qdrant 和 Backend 正在运行。
4. 在左侧导航进入 `导入` 页面，点击 `选择文件` 或 `选择文件夹`。
5. 导入完成后进入 `资料中心` 页面，可以看到资料名、类型、来源、索引状态；图片会显示缩略图。
6. 等待自动索引；也可以在 `任务` 页面点击 `处理索引` 手动触发。
7. 在 `搜索` 页面输入关键词并点击 `搜索`。
...
9. Windows 后端会自动处理索引队列；桌面端 `任务` 页面会显示上传和索引状态。
...
双击 `Vibrary 0.1.9.exe` 后，桌面端会自动启动两个隐藏 sidecar：
...
- `Backend`：显示后端地址。
- `局域网 API`：显示手机可访问的局域网后端地址。默认开启，可在 `配置中心` 中关闭。
- `数据目录`：显示数据目录模式和位置。

### 5.2 页面导航

Windows 左侧是真正的页面导航，不再是同一长页面里的标题跳转：

- `总览`：按步骤显示当前还缺什么，例如服务、局域网、手机、资料导入、embedding、Qdrant 索引和搜索是否可用；未完成步骤会给出下一步按钮。
- `资料中心`：查看所有进入资料组的文件。图片会显示缩略图；每项会显示文件类型、大小、来源设备、索引状态和打开入口。
- `导入`：从 Windows 选择文件或文件夹，导入到资料库正本并进入索引队列。
- `搜索`：搜索已经完成索引的资料。图片结果会显示缩略图，并显示匹配来源和 Qdrant 分数。
- `设备`：显示手机加入用的 6 位验证码、局域网服务地址和可信设备列表。
- `配置中心`：集中调整局域网连接、自动发现广播、自动索引、embedding、检索方式、HNSW 参数、Qdrant、缓存和数据目录。
- `任务`：查看 Android 上传队列和 Windows 后端索引队列，并可手动触发 `处理索引`。
...
- `data/cache/`：应用缓存。
- `data/models/`：Jina / SentenceTransformers 模型缓存。
- `config/`：配置和本地密钥。
...
`资料中心` 是所有可信设备共享的资料列表。只要设备已经加入当前 Windows 后端所在的资料组，就可以查看这里的资料。

资料中心每项资料包含：

- 文件名、文件类型、大小和 MIME 类型。
- 来源设备，例如 Windows 资料库正本、Android 原始文件或缓存副本。
- 索引状态，例如等待索引、索引中、已索引或失败。
- 图片缩略图。当前版本由后端缩略图接口提供图片响应，界面以固定尺寸裁剪显示。
- 打开入口。Windows 本机资料一般会打开后端提供的资料库正本内容地址。

筛选方式：

- 在资料中心顶部输入文件名或类型关键词。
- 使用 `全部类型`、`只看图片`、`只看文档` 切换资料类型。
- 点击 `刷新资料中心` 重新从后端读取最新资料列表。
...
`任务` 页面中的 `上传队列` 显示 Android 上传到 Windows 的任务。常见状态：
...
`任务` 页面中的 `索引队列` 显示等待、正在处理或已完成的索引任务。
...
索引过程中默认使用 `jinaai/jina-embeddings-v5-omni-small` 生成 1024 维向量，并写入 Qdrant。首次使用模型时可能需要下载模型文件，因此第一次索引可能更慢。
...
1. 在输入框中输入关键词。
2. 点击 `搜索`。
3. 结果列表会显示文件标题、缩略图、结果分数、后端推荐的传输方式和本地副本策略。
...
Windows 桌面端会对可访问的结果显示 `打开` 入口。Android 端会根据后端返回的解析策略打开本机原文件、缓存副本或按需下载。

### 5.10 缓存管理

`配置中心` 页面中的缓存区域显示下载缓存大小，并提供：
...
当前版本中，这些页面用于设备管理、模型状态和集中配置：

- `设备`：显示手机加入用的 6 位验证码、当前局域网服务地址和已配对 Android 设备。可以刷新验证码，也可以移除已配对设备。
- `配置中心 / Embedding 阶段`：显示当前 embedding provider。当前可选项为 `jina-embeddings-v5-omni-small`，后续版本可以在同一位置增加更多模型。
- `配置中心 / 检索阶段`：在 `HNSW 向量索引` 和 `遍历 / exact search` 之间切换。遍历模式仍由 Qdrant 执行 exact vector search，不是 SQLite 文件名搜索。
- `配置中心 / HNSW 参数`：调整 `m`、`ef_construct`、`full_scan_threshold` 和查询时的 `hnsw_ef`。
- `配置中心 / 索引控制`：查看资料总数、已索引数量、等待队列、失败队列；可以手动 `处理索引` 或 `重构全部索引`。
- `配置中心 / Qdrant`：查看当前文本、图片语义、图片标签 collection 名称和 point 数量，用于确认搜索是否在预期的 Qdrant collection 上执行。
- `配置中心 / 连接`：提供局域网连接开关、自动发现广播开关和自动索引开关。关闭局域网连接后，手机将无法发现或访问电脑端。

设置会保存到本机配置目录。切换局域网、自动索引、embedding provider、检索方式或 HNSW 参数后，桌面端会自动重启后台服务以应用新设置。切换 embedding 或 HNSW collection 参数后，建议在 `配置中心 / 索引控制` 中点击 `重构全部索引`。

## 6. Android 客户端

Android 默认使用中文界面，底部有六个页面：
...
- 可以在 Windows 桌面端 `配置中心` 关闭局域网连接；关闭后 Android 无法发现或访问电脑端。
...
4. `配置中心` 中是否开启自动处理索引队列；也可以在 `任务` 页面点击 `处理索引` 手动触发。
5. 搜索关键词是否过短或与资料内容无关。
...
1. 启动 Windows 桌面端，确认 `配置中心` 中局域网连接已开启。

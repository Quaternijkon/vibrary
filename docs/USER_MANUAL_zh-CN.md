...
- `desktop/Vibrary 0.1.5.exe`：Windows 桌面程序，双击运行。
...
```powershell
Get-FileHash -Algorithm SHA256 ".\desktop\Vibrary 0.1.5.exe"
...
双击 `Vibrary 0.1.5.exe` 后，桌面端会自动启动两个隐藏 sidecar：
...
- `设置 / 模型`：embedding 模型、版本和本地可用性的入口。
